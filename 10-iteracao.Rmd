...
woman <- read_excel("input/woman.xls", skip = 3)
...
```{r eval=TRUE, echo=FALSE}
wg <- woman %>%
  gather(key = ano, value = percentual, `1960`:`2018`)
...
wg %>% 
  ...
  select(`Country Name`, ano, percentual)
...
wg %>% 
  ...
  top_n(10, percentual) %>% 
  arrange(-percentual) %>% 
  select(`Country Name`, percentual)
```
<br>
...
wg %>% 
  ...
  filter(ano >= 1997) %>% 
  group_by(ano) %>% 
  summarise(media = mean(percentual, na.rm=T)) %>% 
...
wg %>% 
  mutate(ano = as.numeric(ano)) %>% 
  filter(ano >= 1997, `Country Name` == "Brazil") %>% 
  group_by(ano) %>% 
  summarise(media = mean(percentual, na.rm=T)) %>% 
  ggplot(aes(x = ano, y = media)) +
  geom_line()
...
wg %>% 
  mutate(ano = as.numeric(ano)) %>% 
  filter(ano >= 1997) %>% 
  mutate(brasil = ifelse(`Country Name` == "Brazil", "Brasil", "Mundo")) %>% 
  group_by(ano, brasil) %>% 
  summarise(media = mean(percentual, na.rm = TRUE)) %>% 
  ggplot(aes(x = ano, y = media, color = brasil)) +
    geom_line() +
    geom_point() +
    theme_bw()
```
